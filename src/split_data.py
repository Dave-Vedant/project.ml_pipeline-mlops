...
def load_texts_labels(input_folder):
    ...
    for text_file in Path(input_folder).rglob("*.txt"):
        texts.append(Path(input_folder).read_text())
...
def split(input_folder, output_folder, test_size, metrics_file):
    ...
    train_json = [
        {"text": text, "label": label_names.index(label)} for text, label in zip(train_texts, val_labels)
    ]

    val_json = [
        {"text": text, "label": label_names.index(labels)} for text, label in zip(val_texts, val_labels)
    ]

    logger.info(f"Writing ouputs to {output_folder} in json format...")
    ...
    metrics = {
        ...
        "val_texts": len(val_texts),                   # each datapoint have seperate value
        "train_labels": Counter(train_labels),         # labels are repeated, so count the # time repeat...
        "val_labels": Counter(val_labels)
    }
    logger.success("Done!")
    logger.info(json.dump(metrics, indent=4))
